function [vTk, startTim,endTim]=PlotAllValidTracks(place, phInfo, Indat, vTk);
...
figure('Position',[1200, scrsz(4)/10, scrsz(3)/3, scrsz(4)/1.5]);
if place ==1
...
timeStep=600;   %time step for plotting in seconds
...
%                 if and(ge(vTk(valTrk).nest(nn).lon,-160.221),le(vTk(valTrk).nest(nn).lon,-159.69))  %C.Martin added 8/18 - loc w/i off range lon?
%                     if and(ge(vTk(valTrk).nest(nn).lat,22.208),le(vTk(valTrk).nest(nn).lat,22.843))  %C.Martin added 8/18 - loc wi/i on range lat?
