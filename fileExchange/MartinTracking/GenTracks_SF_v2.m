function [vTk, vTkICI, TrackStats, tvcnt, ICI,ICIcor, ICIt, ICItcor, spdICI]=GenTracksSF2(InDir, DeltaTmax, DeltaLatmax, DeltaLonmax, RunPath, fname);
% Generates tracks of consecutive locations, localized by C3D software
% Inputs:
%   InDir = directory where Indat file is saved
%   Indat = NOW A TABLE (was Indat Structure)
%   DeltaTmax = max time between locations to stitch together as one track
%   DeltaLatmax = max lat between locs to include in one track
%   DeltaLonmax = max lon between locs to include in one track
%   RunPath = where to save output file
...
for m=1:loops
    if (BpL.trkd(m)==0) && (BpL.numPh(m)>=minNumPh) && BpL.lse(m)<=minLS
        % sf modified for SCORE - 9/1/2016
        if (ge(BpL.lat(m),32.6)) && (le(BpL.lat(m),33.2)) && (ge(BpL.lon(m),-119.35)) && (le(BpL.lon(m),-118.5))              
            ...
            for tnL=1:999   %magic number MAX number of tracks 999 includes single element tracks!!
                if BpL.trkd(m)==0 && tnum(tnL,2)>0;   %this element NOT tracked yet but the track has elements
                    ...
                    if ((BpL.tim(m)-Tk(tnL).nest(length(mm)).tim) <= DeltaTmax && abs(Indat(m).lat-Tk(tnL).nest(length(mm)).lat)< DeltaLatmax && abs(Indat(m).lon-Tk(tnL).nest(length(mm)).lon)<DeltaLonmax )
                        Indat(m).trkd=1;
                    ...
                    if BpL.trkd(m)==0 && tnum(tnL,2)==0;  %this element not already tracked
                        BpL.trkd(m)=1;    %set m trkd =1
                        tnum(tnL,2)=1;      %set trkd element cntr=1
                        tnum(tnL,3)=m;      %set m of first element in trk=m
                        mm=tnum(tnL,3:2+tnum(tnL,2));
                        ntc=tnum(tnL,1);
                        Tk(tnum(ntc,1))=struct('trnum', ntc ,'nest', struct(BpL(mm,:))); %was tnum(1,1) changed to ntc
                        break
                    end
                end
            end
