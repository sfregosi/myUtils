function [vTk, vTkICI, TrackStats, tvcnt, ICI,ICIcor, ICIt, ICItcor, spdICI]=GenTracks_SF(InDir, DeltaTmax, DeltaLatmax, DeltaLonmax, RunPath, fname);
...
for m=1:loops
    if (Indat(m).trkd==0) && (Indat(m).numPh>=minNumPh) && Indat(m).lse<=minLS
        ...
        if (ge(Indat(m).lat,32.6)) && (le(Indat(m).lat,33.2)) && (ge(Indat(m).lon,-119.35)) && (le(Indat(m).lon,-118.5))   
            ...
            for tnL=1:20000   %magic number MAX number of tracks 999 includes single element tracks!!
