...
filesize = getfield(dir(xwav),'bytes');
...
elseif ~strcmp(PARAMS.xhd.hSubchunkID,'harp')
    ...
    disp(['SubchunkID = ',PARAMS.xhd.hSubchunkID])
    return
end
...
if PARAMS.xhd.hSubchunkSize ~= (64 - 8 + PARAMS.xhd.NumOfRawFiles * 32)
    ...
    disp(['hSubchunkSize = ',num2str(PARAMS.xhd.hSubchunkSize)])
    disp(['NumOfRawFiles = ',num2str(PARAMS.xhd.NumOfRawFiles)])
 %   return
end
...
%         + [0 0 0 0 0 (PARAMS.xhd.byte_length(i) - 2)  ./  PARAMS.xhd.ByteRate];
    PARAMS.raw.dnumEnd(i) = PARAMS.raw.dnumStart(i) ...
        + datenum([0 0 0 0 0 (PARAMS.xhd.byte_length(i) - 2)  ./  (PARAMS.xhd.ByteRate * PARAMS.xhd.NumChannels)]);
    PARAMS.raw.dvecEnd(i,:) = PARAMS.raw.dvecStart(i,:) ...
        + [0 0 0 0 0 (PARAMS.xhd.byte_length(i) - 2)  ./  (PARAMS.xhd.ByteRate * PARAMS.xhd.NumChannels)];
end
...
if ~strcmp(PARAMS.xhd.dSubchunkID,'data')
    ...
    disp(['SubchunkID = ',PARAMS.xhd.dSubchunkID])
    return
end
...
% PARAMS.start.dvec(1) = PARAMS.xhd.year(1);
% PARAMS.start.dvec(2) = PARAMS.xhd.month(1);
% PARAMS.start.dvec(3) = PARAMS.xhd.day(1);
% PARAMS.start.dvec(4) = PARAMS.xhd.hour(1);
% PARAMS.start.dvec(5) = PARAMS.xhd.minute(1);
% PARAMS.start.dvec(6) = PARAMS.xhd.secs(1) + PARAMS.xhd.ticks(1)/1000;
% 
...
if display_times
    year = PARAMS.xhd.year;
    month = PARAMS.xhd.month;
    day = PARAMS.xhd.day;
    hour = PARAMS.xhd.hour;
    min = PARAMS.xhd.minute;
    secs = PARAMS.xhd.secs;
    ticks = PARAMS.xhd.ticks;
    
    
    for rf = 1:length(PARAMS.xhd.year)
        xdatevec = [ year(rf)+2000 month(rf) day(rf) hour(rf) min(rf)...
            secs(rf)+ticks(rf)/1000 ];
        disp(datestr(xdatevec));
    end
end

% PARAMS.end.dvec = datevec(datenum(PARAMS.start.dvec + [0 0 0 0 0 PARAMS.end.sample/PARAMS.fs]));