...
dasbrList = dir([drive cruise '_DASBR\DS*']);

for d = 1:length(dasbrList)
    dasbrNum = dasbrList(d).name;
    fprintf(1, 'Starting %s\n', dasbrNum)
    stFolder = dir([drive cruise '_DASBR\' dasbrNum '\ST*']);
    stNum = stFolder.name;
    srFolder = dir([drive cruise '_DASBR\' dasbrNum '\' stFolder.name]);
    ...
    path_raw = [drive cruise '_DASBR\' dasbrNum '\' stNum '\' serial '\'];
    % cd(path_raw);
    ...
    elseif ~isempty(wavFiles)
        ...
        for f = 1:length(fsNew)
            ...
            if rem(dfN,1) == 0
                ...
                path_outN = [drive cruise '_DASBR\decimated\' ...
